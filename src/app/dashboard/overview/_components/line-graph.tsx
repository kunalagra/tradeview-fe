...
export function LineGraph() {
  ...
  useEffect(() => {
    const fetchIndices = async () => {
      try {
        const socket = new WebSocket(process.env.NEXT_PUBLIC_WEBSOCKET_URL || ''); // Adjust URL to match your Fastify WebSocket URL
        socket.onopen = () => {
          socket.send(JSON.stringify({ type: "getIndices" })); // Ask for available indices
        };
        socket.onmessage = (event) => {
          ...
          if (data.type === "indices") {
            ...
            if (data.data.length > 0) {
              setSelectedIndex(data.data[0]); // Automatically select the first index
            }
          }
        };
        setWs(socket);
